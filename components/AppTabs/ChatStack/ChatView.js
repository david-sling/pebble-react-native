...
export default function ChatView( { route:{params:{chat, participants }}, tokenProps, userProps } ) {
    ...
    const getMessages = async () => {
        ...
        const result = await axios.post(`${config.SERVER_URL}/messages/get/${chat.item._id}`, {token} )
        ...
        <View style={{flex:1}} >
            ...
            <View style={styles.messages} >
                {messages&&<FlatList 
                    ...
                    data={messages}
                    renderItem={ (message) =>(
                        <View style={[styles.message, message.item.sender==userProps.user._id&&styles.user]} >
                            <Text>{participants.find(participant=> participant._id==message.item.sender).name}</Text>
                ...
                <View style={styles.messageInput} >
                    <TextInput 
                        ...
                        onContentSizeChange={(event) => {
                            const lines = Math.floor(event.nativeEvent.contentSize.height/17)
                            const max = 5
                            // console.log(lines)
